...
**Crossum COMPLETE — game playable.** Built `games/kakuro/kakuro.js` (~640 lines, adapted from `suguru.js`): bank loader + played-set tracking (`kakuro_bank_version`), run structure derived from the `cells` array (`buildRuns`, length≥2 only — a length-1 sequence carries no clue), grid render (black walls with diagonal slash + corner sums, white cells, **given cells locked/dimmed & non-editable**), tap-to-select with across/down **run-highlight**, 1–9 picker + pencil + erase, picker auto-dim of run-mates (gated on the "Highlight runs" toggle = `autoDisable` key), undo, timer, pause, restart, give-up, full-grid auto-check → win modal + coin reward (`{easy:4,medium:8,hard:14}`), hint shop (Random 2c / Chosen 5c), 5-slide Kakuro tutorial with mini-grid visuals, settings, daily overlay — all shared via `profile.js`. CSS added `.cell.given` and `.cell.same-number`. Verified: across all 400 puzzles runs reconstruct correctly (0 bad, 0 uncovered cells); full correct fill → win; one wrong cell → no win + error mark; headless-Chrome screenshots confirm correct rendering at easy/medium/hard. The Crossum tile in `games/index.html` already linked to `/games/kakuro/` (not coming-soon) — no change needed. **All 5 number/word games now built: Sudoku, Nettle, Pixle, Wordle, Crossum** (Threadle/Masyu remains a stub).
